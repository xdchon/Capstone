...
class ModelButton(QPushButton):
    ...
    def press(self, parent):
        ...
        if hasattr(parent, "segmentation_settings") and getattr(
            ...
            parent.compute_segmentation_all_timepoints(custom=False)
        else:
            # run single-timepoint segmentation asynchronously so GUI stays responsive
            seg_T = None
            if getattr(parent, "has_time", False) and hasattr(parent, "currentT"):
                try:
                    seg_T = int(parent.currentT)
                except Exception:
                    seg_T = None
            parent.start_segmentation_async(model_name="cpsam", time_index=seg_T)
...
class SegmentationSettings(QWidget):
    ...
    def __init__(self, font):
        ...
        ########################### Downscale & time-lapse options ###########################
        down_qlabel = QLabel("downscale XY:")
        down_qlabel.setToolTip(
            "factor to resize XY before segmentation (e.g. 0.5 halves width/height). "
            "Use 1.0 for no downscaling; values <1 speed up segmentation at the cost of resolution."
        )
        down_qlabel.setFont(font)
        grid_layout.addWidget(down_qlabel, row, 0, 1, 4)
        self.downscale_box = QLineEdit()
        self.downscale_box.setText("1.0")
        self.downscale_box.setFixedWidth(40)
        self.downscale_box.setFont(font)
        self.downscale_box.setToolTip(
            "factor to resize XY before segmentation (0.1–1.0). 1.0 = no downscale."
        )
        grid_layout.addWidget(self.downscale_box, row, 4, 1, 2)
    ...
    def downscale_factor(self):
        """Return XY downscale factor in (0,1], default 1.0 if invalid."""
        text = self.downscale_box.text().strip()
        try:
            val = float(text)
        except ValueError:
            val = 1.0
        ...
        if val > 1.0:
            val = 1.0
        # normalize text to the clamped value
        self.downscale_box.setText(f"{val:.3f}".rstrip("0").rstrip("."))
        return val
