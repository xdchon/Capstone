...
def _convert_image_3d(x, channel_axis=None, z_axis=None, time_axis=None, pad_channels=True):
    """
    Convert a 3D, 4D, or 5D image array to have dimensions ordered as (Z, X, Y, C) or (T, Z, X, Y, C).
    ...
    Arrays of ndim=5 are assumed to contain a time dimension. time_axis will be used if provided,
    otherwise the leading dimension is treated as time.
    
    Args:
        x (numpy.ndarray): Input image array. Must be either 3D (assumed to be grayscale 3D), 4D, or 5D.
        ...
        time_axis (int): The axis index corresponding to the time dimension for 5D images. If None and \
            x.ndim==5, the leading axis is treated as time.

    Returns:
        numpy.ndarray: A 4D image array with dimensions ordered as (Z, X, Y, C) (or 5D (T, Z, X, Y, C) \
        for time series), where C is the channel dimension. If the input has fewer than 3 channels, the \
        output will be padded with zeros to have 3 channels. If the input has more than 3 channels, only \
        the first 3 channels will be retained.
    ...
    if x.ndim == 5:
        ...
        if time_axis < 0:
            time_axis += x.ndim
        axes_all = list(range(x.ndim))
        keep_axes = set([time_axis, z_axis, channel_axis])
        xy_axes = [ax for ax in axes_all if ax not in keep_axes]
        if len(xy_axes) != 2:
            raise ValueError("5D images must have exactly two spatial axes in addition to time, channel, and z axes")
        # order: T, Z, Y, X, C (keep XY order as encountered)
        transpose_order = (time_axis, z_axis, xy_axes[0], xy_axes[1], channel_axis)
    ...
    if num_channels != 3:
        ...
        if num_channels > 3:
            ...
            if pad_channels:
                pad_width = [(0, 0)] * (x.ndim - 1) + [(0, 3 - x_chans_to_copy)]
                x = np.pad(x, pad_width, mode='constant', constant_values=0)
            else:
                # leave as-is; downstream will pad per-chunk to avoid massive allocations
                pass

    return x


def convert_image(x, channel_axis=None, z_axis=None, time_axis=None, do_3D=False):
    ...
    Args:
        ...
        time_axis (int or None): The axis of the time dimension for 5D inputs. If None and ndim==5, the leading axis is used.
    ...
    # for 2D / 2D+stitch modes, ignore any z_axis hint and treat the first
    # dimension as a batch axis; CLI and GUI may pass z_axis when using
    # stitch_threshold>0 on 3D stacks
    if z_axis is not None and not do_3D:
        transforms_logger.warning(
            "z_axis was specified but do_3D is False; ignoring z_axis and treating input as a stack of 2D images"
        )
        z_axis = None

    # make sure that channel_axis and z_axis are specified if 3D
    if do_3D:
        pad_channels = False if x.ndim == 5 else True
        return _convert_image_3d(x, channel_axis=channel_axis, z_axis=z_axis, time_axis=time_axis, pad_channels=pad_channels)
